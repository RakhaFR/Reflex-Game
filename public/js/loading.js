/* ==========================================================================
   REFLEXRHYTHM — LOADING.JS  (FIXED: double-wipe & reveal delay)
...
(function ReflexLoadingEngine() {
  ...
  function startTicker() {
    ...
    _ticker = setInterval(() => {
      if (_displayPct < _targetPct) {
        _displayPct = Math.min(
          _targetPct,
          _displayPct + Math.ceil((_targetPct - _displayPct) * 0.2) + 1,
        );
      }
  ...
  const _fromTransition = (() => {
    try {
      ...
      if (flag) {
        sessionStorage.removeItem("rr_transition");
        return true;
      }
    } catch (_) {}
    return false;
  ...
  if (!_fromTransition) {
    ...
    if (document.body) {
      ...
      document.addEventListener("DOMContentLoaded", playTransitionScreen, {
        once: true,
      });
    }
  }
