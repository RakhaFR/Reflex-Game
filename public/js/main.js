...
const UPDATE_LOGS = [
  {
    ...
    changes: [
      ...
      {
        type: "upd",
        text: "Penataan ulang layout menu & gameplay dengan konsep *Stylized Arcade Interface*. masih tahap pre-test kemungkinan data anda bakal hilang di update selanjutnya..",
      },
    ],
  },
  // {
...
function initLobbyPageLogic() {
  ...
  const getSongItems = () =>
    Array.from(document.querySelectorAll(".song-item"));
  ...
  function renderBannerSwatches(activeBannerId) {
    ...
    BANNER_SKINS.forEach((b) => {
      ...
      btn.className =
        "banner-swatch-btn" + (b.id === activeBannerId ? " active" : "");
      btn.title = b.label;
  ...
  window.syncLobbyProfileDOM = function syncLobbyProfileDOM() {
    ...
    const expWidthPercentage = Math.min(
      (currentExpInLevel / xpThisLevel) * 100,
      100,
    );
    ...
    if (widgetLevelNumber)
      widgetLevelNumber.textContent = isMaxLevel
        ? "MAX"
        : "LV " + computedLevel;
    if (widgetXpBarFill) widgetXpBarFill.style.width = `${expWidthPercentage}%`;
    if (widgetUsername)
      widgetUsername.textContent = profile.identity.username.toUpperCase();
    if (widgetAvatar) {
      const avatarSrc =
        typeof getAvatarDisplay === "function"
    ...
    if (typeof applyWidgetBanner === "function")
      applyWidgetBanner(activeBannerId);
    if (typeof renderBannerSwatches === "function")
      renderBannerSwatches(activeBannerId);
    ...
    if (modalUsernameText)
      modalUsernameText.textContent = profile.identity.username.toUpperCase();
    ...
    const bm = profile.stats.basic || {
      clicks: 0,
      wrongClicks: 0,
      gamesPlayed: 0,
    };
    const nom = profile.stats.notoriginal || {
      clicks: 0,
      wrongClicks: 0,
      gamesPlayed: 0,
    };
    ...
    $set(
      "modalStatHighBasic",
      (profile.stats.records.highestBasicScore || 0).toLocaleString(),
    );
    if (document.getElementById("modalStatHighNotOriginal"))
      $set(
        "modalStatHighNotOriginal",
        (profile.stats.records.highestNotOriginalScore || 0).toLocaleString(),
      );
    $set(
      "statTotalAccuracy",
      calcAcc(
        profile.stats.totalClicks || 0,
        profile.stats.totalWrongClicks || 0,
      ),
    );
    ...
    if (bmTotal > 0) {
      setBar(
        "graphBmHit",
        "graphBmHitVal",
        Math.round((bm.clicks / bmTotal) * 100),
      );
      setBar(
        "graphBmWrong",
        "graphBmWrongVal",
        Math.round((bm.wrongClicks / bmTotal) * 100),
      );
    } else {
    ...
    if (nomTotal > 0) {
      setBar(
        "graphNomHit",
        "graphNomHitVal",
        Math.round((nom.clicks / nomTotal) * 100),
      );
      setBar(
        "graphNomWrong",
        "graphNomWrongVal",
        Math.round((nom.wrongClicks / nomTotal) * 100),
      );
    } else {
    ...
    const totalActions =
      (profile.stats.totalClicks || 0) +
      (profile.stats.totalBonusTriggered || 0) +
      (profile.stats.totalWrongClicks || 0);
    if (totalActions > 0) {
      setBar(
        "graphBarClicks",
        "graphValClicks",
        Math.round(((profile.stats.totalClicks || 0) / totalActions) * 100),
      );
      setBar(
        "graphBarBonus",
        "graphValBonus",
        Math.round(
          ((profile.stats.totalBonusTriggered || 0) / totalActions) * 100,
        ),
      );
      setBar(
        "graphBarWrong",
        "graphValWrong",
        Math.round(
          ((profile.stats.totalWrongClicks || 0) / totalActions) * 100,
        ),
      );
    } else {
    ...
    syncToggleState(
      "particleToggleBtn",
      profile.settings.particleEffectEnabled,
    );
    syncToggleState(
      "comboAnimToggleBtn",
      profile.settings.comboAnimationEnabled,
    );
    syncToggleState(
      "mouseClickToggleBtn",
  ...
  if (changeAvatarBtn && avatarFileInput) {
    ...
    avatarFileInput.addEventListener("change", (e) => {
      ...
      if (!file.type.startsWith("image/")) {
        showLobbyToast("File harus berupa gambar!", "error");
        avatarFileInput.value = "";
        return;
      }
      if (file.size > 8 * 1024 * 1024) {
        showLobbyToast("File terlalu besar! Maks 8MB", "error");
        avatarFileInput.value = "";
        return;
      }
      ...
      reader.onload = (ev) => {
        ...
        img.onerror = () => {
          showLobbyToast("Gagal baca gambar!", "error");
          avatarFileInput.value = "";
        };
        img.src = ev.target.result;
      };
      reader.onerror = () => {
        showLobbyToast("Gagal baca file!", "error");
        avatarFileInput.value = "";
      };
  ...
  // Reset data
  document
    .getElementById("resetProfileDataBtn")
    ?.addEventListener("click", (e) => {
      e.stopPropagation();
      triggerLobbyClickSound();
      if (
        !confirm("Reset SEMUA data profile?\nAksi ini tidak bisa dibatalkan.")
      )
        return;
  ...
  function openProfileModal() {
    ...
    const identityBtn = document.querySelector(
      '.modal-tab-btn[data-tab="tabIdentity"]',
    );
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (typeof profile !== "undefined") {
      if (!Array.isArray(profile.settings.keybinds))
    ...
    renderKeybindEditor();
    showLobbyToast(
      `Key ${KEYBIND_LABELS[keybindListeningIdx < 0 ? 0 : keybindListeningIdx]} → "${k.toUpperCase()}"`,
      "success",
    );
  });
  ...
  function triggerLobbyClickSound() {
    const sfx =
      document.getElementById("lobbyClickSfx") ||
      document.getElementById("clickSound");
  ...
  function changeTrack(index) {
    ...
    const activeTrackList = (() => {
      const modeId =
        typeof BM_GAME_MODES !== "undefined" &&
        BM_GAME_MODES[bmGameModeIdx]?.id;
      if (modeId === "notoriginal" && typeof NOM_TRACKS !== "undefined")
        return NOM_TRACKS;
    ...
    if (meta) {
      ...
      if (displayNum)
        displayNum.textContent = String(currentIdx + 1).padStart(2, "0");
      if (displayRole) displayRole.textContent = meta.role || "";
      if (displayTitle) {
        displayTitle.textContent = meta.title;
        displayTitle.className = `mode-title ${meta.titleClass || "title-basic"}`;
      }
    ...
    const activeModeId =
      typeof BM_GAME_MODES !== "undefined" && BM_GAME_MODES[bmGameModeIdx]?.id;
  ...
  function switchGameMode(dir, isInit = false) {
    ...
    if (!isInit) {
      bmGameModeIdx =
        (bmGameModeIdx + dir + BM_GAME_MODES.length) % BM_GAME_MODES.length;
    ...
    newSongItems.forEach((item, i) => {
      item.addEventListener("click", () => {
        if (typeof triggerLobbyClickSound === "function")
          triggerLobbyClickSound();
        if (typeof stopBgMusic === "function") stopBgMusic();
    ...
    if (isInit) {
      ...
      nextIdx =
        !isNaN(savedTrack) &&
        savedTrack >= 0 &&
        savedTrack < newSongItems.length
          ? savedTrack
          : 0;
    } else {
    ...
    if (newSongItems[currentIdx]) {
      if (typeof startPreview === "function")
        startPreview(newSongItems[currentIdx]);
    }
  }
  ...
  function handlePlayButtonClick() {
    ...
    if (typeof engineFn === "function") {
      ...
      console.warn(
        `Engine ${engineFnName} belum siap. Fallback otomatis ke Basic Mode.`,
      );
      if (typeof startBasicMode === "function") startBasicMode();
    }
  }
  ...
  window.addEventListener("keydown", (e) => {
    if (
      typeof lobbyProfileModal !== "undefined" &&
      lobbyProfileModal?.classList.contains("active")
    ) {
