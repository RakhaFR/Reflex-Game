// ============================================================
// REFLEXRHYTHM — BASIC MODE (OSU-STYLE ENGINE v2)
...
function bmGameOver() {
  ...
  if (popup) {
    const track =
      typeof BM_TRACKS !== "undefined" ? BM_TRACKS[finalTrackIdx] : null;
    const clicks =
      typeof profile !== "undefined" && profile.stats?.basic
...
function startBasicMode() {
  ...
  if (urlMode === "notoriginal") {
    console.warn(
      "startBasicMode() dipanggil saat mode=notoriginal, skip ke NOM.",
    );
  ...
  if (document.getElementById("lobbyProfileWidget") !== null) {
    ...
    if (typeof rrNavigate === "function") {
      rrNavigate(`game.html?mode=${currentModeId}&diff=${diff}&track=${track}`);
    } else {
  ...
  showCountdownThen(() => {
    ...
    if (_isTouchDevice) {
      ...
      document.addEventListener("touchstart", bmTouchHandler, {
        passive: false,
      });
    }
